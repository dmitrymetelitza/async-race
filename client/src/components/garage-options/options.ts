...
export class GarageOptions extends Component {
  ...
  constructor(parentNode: HTMLElement) {
    ...
    const createInputs = new OptionsInputs(this.element, 'create', ['garage-create-inputs']);
    createInputs.createCar = (state) => this.createCar(state);

    this.updateInputs = new OptionsInputs(this.element, 'update', ['garage-update-inputs']);
